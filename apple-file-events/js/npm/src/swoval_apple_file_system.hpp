...
public:
  ...
  int startStream(const char *path, double latency, int flags) {
    ...
    for (auto pair = stream_handles.begin(); pair != stream_handles.end();
         ++pair) {
      if (!memcmp(pair->second.first.data(), path,
                  pair->second.first.length())) {
        lock.unlock();
        return pair->first;
      }
    }

    CFStringRef mypath = CFStringCreateWithCStringNoCopy(
        nullptr, path, kCFStringEncodingUTF8, nullptr);
  ...
  static void defaultCallback(ConstFSEventStreamRef stream, void *info,
...
template <typename T> static void cleanupFunc(handle<T> *h) {
  ...
  for (auto pair = h->stream_handles.begin(); pair != h->stream_handles.end();
       ++pair) {
