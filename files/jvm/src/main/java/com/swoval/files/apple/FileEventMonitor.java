...
public interface FileEventMonitor extends AutoCloseable {
   ...
   *
   * @param latency the minimum latency with which the os will deliver file events. It seems to
   * be limited to roughly O(10ms), so there is little point in setting the value less than that.
   ...
   */
  FileEventMonitors.Handle createStream(Path path, int latency, TimeUnit timeUnit, Flags.Create flags);

  /**
   * Stop monitoring a path previously registered with {@link FileEventMonitor#createStream(Path, int, TimeUnit, Create)}.
   * @param streamHandle the handle returned by {@link FileEventMonitor#createStream(Path, int, TimeUnit, Create)}
   */
