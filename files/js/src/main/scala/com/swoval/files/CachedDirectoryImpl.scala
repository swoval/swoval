...
/**
  ...
  */
class CachedDirectoryImpl[T](@BeanProperty val path: Path,
  ...
  override def list(maxDepth: Int,
                    filter: Filter[_ >: TypedPath]): List[TypedPath] =
  ...
  override def listEntries(maxDepth: Int,
                           filter: Filter[_ >: Entry[T]]): List[Entry[T]] =
  ...
  /**
    ...
    */
  override def update(typedPath: TypedPath,
                      thread: Executor.Thread): Updates[T] =
  ...
  private def addDirectory(currentDir: CachedDirectoryImpl[T],
    ...
    if (exists) {
      ...
      if (previous != null) {
        oldEntries.put(previous.realPath,
                       Entries.setExists(previous.getEntry, false))
        val entryIterator: Iterator[Entry[T]] =
          previous.listEntries(java.lang.Integer.MAX_VALUE, AllPass).iterator()
        while (entryIterator.hasNext) {
          val entry: Entry[T] = entryIterator.next()
...
// We will always return from this block
          synchronized(currentDir.lock) {
            ...
            if (!isDirectory || currentDir.depth <= 0 || isLoop(resolved,
              ...
              if (oldResolvedEntry == null) {
                ...
                result.onUpdate(oldResolvedEntry,
                                Entries.resolve(currentDir.path, newEntry))
              }
              result
            } else {
  ...
  private def findImpl(
      parts: List[Path]): Either[Entry[T], CachedDirectoryImpl[T]] = {
    val it: Iterator[Path] = parts.iterator()
    var currentDir: CachedDirectoryImpl[T] = this
          ...
          if (pathFilter.accept(file)) {
            ...
            if (file.isDirectory) {
              if (depth > 0) {
                ...
                files.put(key,
                          Entries.get(TypedPaths.getDelegate(key, file),
                                      converter,
                                      file))
              }
            } else {
